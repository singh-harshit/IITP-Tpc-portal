...
export class CoordinatorJobEligibility extends React.Component
{
  constructor(props){
    super(props);
  ...
  this.state = {
    ...
    handleSubmit = async (event) =>{
      ...
      axios({
        url: `/backend/coordinator/jobs/updateEligibilityCriteria/${this.state.id}`,
        method: 'patch',
        data: payload,
        headers: {
          'x-auth-token': this.state.authToken,
      ...
      .then((s)=>{
        this.setState({
          redirect:'/coordinator/job/'+this.state.id
        })
      })
      .catch((e)=>{
  ...
  <div className="container-fluid row mt-2">
    ...
    <div className="col-md-3">
      <Link style={{ textDecoration: 'none', color: 'white' }} to={"/coordinator/job/"+this.state.id}><button type="button" class="btn btn-outline-dark btn-block m-1 col-md-12">Back</button></Link>
    </div>
  </div>
    </div>
  );
